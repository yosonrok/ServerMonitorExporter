...
def update_metrics(config, metrics):
    ...
    firewall_counter = 0
    # 儲存上一次的 I/O 數據
    last_io_counters = {proc_name: {'read': 0, 'write': 0, 'time': time.time()} for proc_name in process_names}
        ...
        for proc_name in process_names:
            ...
            for proc in psutil.process_iter(['name', 'cpu_percent', 'memory_info', 'io_counters']):
                if proc.info['name'] == proc_name:
                    ...
                    last_data = last_io_counters[proc_name]
                    time_diff = current_time - last_data['time']
                    if time_diff > 0:
                        read_rate = (proc.info['io_counters'].read_bytes - last_data['read']) / time_diff
                        write_rate = (proc.info['io_counters'].write_bytes - last_data['write']) / time_diff
                        metrics['disk_read_rate'].labels(server=server_name, process_name=proc_name).set(read_rate)
                        metrics['disk_write_rate'].labels(server=server_name, process_name=proc_name).set(write_rate)
                    # 更新上一次的 I/O 數據
                    last_io_counters[proc_name] = {
                        'read': proc.info['io_counters'].read_bytes,
                        'write': proc.info['io_counters'].write_bytes,
                        'time': current_time
                    }
                    break
            if not found:
                ...
                metrics['disk_read_rate'].labels(server=server_name, process_name=proc_name).set(0)
                metrics['disk_write_rate'].labels(server=server_name, process_name=proc_name).set(0)
